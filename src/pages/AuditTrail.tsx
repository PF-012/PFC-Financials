...
export default function AuditTrail() {
  ...
  useEffect(() => {
    ...
    setError('');
    const q = query(collection(db, 'audit_logs'), where('companyId', '==', activeCompany.id));
    const unsub = onSnapshot(q, (snapshot) => {
      const docs: AuditLog[] = snapshot.docs.map((d: any) => ({ ...d.data(), id: d.id }));
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-3 border-b border-gray-200 pb-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Chronological record of recorded modifications for this company.</p>
        </div>
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {logs.length === 0 ? (
                <tr><td colSpan={4} className="px-6 py-8 text-center text-sm text-gray-500">No audit logs found.</td></tr>
              ) : logs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-mono">
                    {new Date(log.timestamp).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short', timeZone: 'UTC' })}
                  </td>
